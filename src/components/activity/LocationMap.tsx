...
export function LocationMap({ latitude, longitude, name }: LocationMapProps) {
  ...
  const staticMapUrl = `https://api.mapbox.com/styles/v1/mapbox/streets-v12/static/pin-s+${markerColor}(${longitude},${latitude})/${longitude},${latitude},${zoom},0/${width}x${height}@2x?access_token=${MAPBOX_TOKEN}`;

  const handleClick = () => {
    // Navigate to map page with venue coordinates as query params
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full rounded-xl overflow-hidden border border-border hover:border-primary/50 transition-colors group cursor-pointer"
