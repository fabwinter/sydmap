-- Update the bulk import function to handle the JSON format with lat/lon and amenities array
CREATE OR REPLACE FUNCTION public.bulk_import_activities_v2(activities_json JSONB)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    -- Convert amenities JSONB array to text array
    SELECT array_agg(elem::TEXT) INTO amenities_arr
    FROM jsonb_array_elements_text(COALESCE(activity->'amenities', '[]'::JSONB)) AS elem;
    ...
    INSERT INTO activities (
      ...
      activity->>'name',
      activity->>'category',
      COALESCE((activity->>'lat')::NUMERIC, (activity->>'latitude')::NUMERIC),
      COALESCE((activity->>'lon')::NUMERIC, (activity->>'longitude')::NUMERIC),
      (activity->>'rating')::NUMERIC,
      COALESCE((activity->>'reviews_count')::INTEGER, (activity->>'review_count')::INTEGER, 0),
